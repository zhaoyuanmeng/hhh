<template>
  <div class="childbox">
    ...
    <div class="tablebox">
      <el-table :data="tableData" highlight-current-row style="width: 100%; height: 80%;background: #021B4F;"
        ...
        <el-table-column prop="roleName" label="角色名称" min-width="12%" align="center" />
  ...
  <el-dialog v-model="addUser" width="714px" @close="cancel" align-center :destory-on-close="true"
    ...
    <template #header>
      <div class="heard_name">
        <div class="d_name">{{ titleType === 'add' ? "新增角色" : "编辑角色" }}</div>
      </div>
    </template>
    <el-form ref="customForm" :inline="true" :model="form" :label-suffix="'：'" :label-width="100" class="customForm"
      ...
      <el-row>
        ...
        <el-col :span="24">
          <el-form-item label="权限范围" style="width: 100%" prop="resources" :rules="[
            { required: true, message: '必填', trigger: 'change' },
          ]">
            <el-checkbox-group v-model="form.resources" >
              <el-checkbox v-for="city in menuList" :key="city.code" :value="city.name">{{
                city.name
              }}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
  ...
  <el-dialog v-model="userRole" width="680px" align-center :destory-on-close="true" :close-on-click-modal="false"
    class="my_Dialog">
    <template #header>
      <div class="heard_name">
    ...
    <div class="role_Box">
      <div class="item">
        <div class="label_class">角色名称</div>
        <div class="right_data">{{ selectRow.roleName }}</div>
      </div>
      <div class="item">
        <div class="label_class">权限范围</div>
        <div class="right_data">
          <div class="item_box" v-for="(item,index) in selectRow.resources.split(',')" :key="index">{{ item }}</div>
        </div>
      </div>
    </div>
  </el-dialog>
...
import { getRolePageList, addRole, updateRole, delRole } from '@/api/system'
  ...
  proxy.$refs["customForm"].validate(valid => {
    if (valid) {
      if (titleType.value === 'edit') {
        // 编辑
        form.resources = form.resources.join(',')
...
const getUserListData = () => {
  getRolePageList(query).then(res => {
    if (res.code === 0) {
...
function edit(row) {
  ...
  form.resources = row.resources?row.resources.split(','):[]
  form.id = row.id;
...
function deleterow(row) {
  ...
  ElMessageBox.confirm("确认要删除此用户信息吗？", "提示", {
    ...
    .then(async () => {
      let result = await delRole({ id: row.id });
      if (result.code === 0) {
        getUserListData();
        ElMessage({
          type: "success",
