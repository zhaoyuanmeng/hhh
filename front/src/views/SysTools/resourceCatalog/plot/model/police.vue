      ...
      <div class="func-warp">
        <el-scrollbar>
          <div v-for="(item, index) in dataList"
            :key="index"
            @click.stop="handleClickLabel(item, index)"
            @dblclick.stop="handleDbClick(item, index)">
            <div
            :class="['label-warp-list', { active: index == activeName }]" v-if="groupId===item.groupId">
            ...
            <el-dropdown placement="left" @command="handleCommand">
              ...
              <template v-slot:dropdown>
                <el-dropdown-menu class="cloud-dropdown">
                  <el-dropdown-item
                    :command="{ type: '编辑', item: item, index }"
                    >编辑</el-dropdown-item
                  >
                  <el-dropdown-item
                    :command="{ type: '删除', item: item, index }"
                    >删除</el-dropdown-item
                  >
                </el-dropdown-menu>
              </template>
        ...
        <div class="warp-footer">
          <div @click="handleLabelAdd">新增模型</div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    async handleCommand(command) {
      if (command.type === "编辑") {
        ...
        this.isShowAdd = true
        // window.sealAPI._tag.delete_biz(command.item);
    ...
    async setData() {
      ...
      let datas = sessionStorage.get('QXZS_3dMarker')
      // let items = datas.filter((item) => item.groupId !== "police") || [];
      this.dataList = datas;
    },
    async handleClose() {
      this.$emit("close");
