            ...
            <el-form @submit.prevent :model="fModel" :rules="fRules" ref="fForm" size="mini" label-position="left"
                ...
                <el-scrollbar>
                    ...
                    <el-form-item prop="list" class="warp-l">
                        <div v-for="(item, index) in markerNameList" :key="index" class="item"
                            @click="handleClickMarker(item)">
                            <span>{{ item.text }}</span>
                        </div>
                        <div v-if="markerNameList.length == 0" style="width: 100%;text-align: center;color: #b7bbc2;">无数据！
                        </div>
                    </el-form-item>

                </el-scrollbar>
                <el-form-item class="btn">
                    <!-- <el-button type="primary" v-if="!showR" @click="handleInquire">查询</el-button> -->
                    <!-- <el-button type="primary" v-else @click="handleRedraw">重新绘制</el-button>
                    <el-button v-show="rList.length != 0" @click="exportToExcel">结果导出</el-button> -->
...
import { pointAndPolygonInfo, pointName } from './coordianteData'
import { getPoiData } from '@/api/dataApi'
...
export default {
    ...
    mounted() {
        getPoiData().then((res) => {
            pointAndPolygonInfo.point.coordinates = []
            res.features.forEach(element => {
                pointAndPolygonInfo.point[0].coordinates.push([element.geometry.x, element.geometry.y, 0])
                this.coordinates.push([element.geometry.x, element.geometry.y, 0])
                pointName.push(element.attributes.名称)
            })
        })
    },
    methods: {
        ...
        async handleChangeBasic(editHelperCoordinates) {
            ...
            // const markerArr = []
            this.markerArr = []
            // const res = await __g.coord.pcs2gcs(editHelperCoordinates)
            // let gcsPolygonCoordinates = []
            let gcsPolygonCoordinates = editHelperCoordinates
            gcsPolygonCoordinates.push(editHelperCoordinates[0])
            // if (res.coordinates) {
            //     gcsPolygonCoordinates = res.coordinates.map(item => [item[0], item[1]])
            //     gcsPolygonCoordinates.push(gcsPolygonCoordinates[0])
            // }

            pointAndPolygonInfo.point.forEach(async (item, index) => {
                // const res = await __g.coord.pcs2gcs(item.coordinates)
                // const gcsPointCoordiantes = res.coordinates.map((item) => {
                const gcsPointCoordiantes = item.coordinates.map((item) => {
                    return [item[0], item[1]]
                })
                gcsPointCoordiantes.forEach((pointCoordinate, i) => {
                    const pt = turf.point(pointCoordinate)
                    const poly = turf.polygon([gcsPolygonCoordinates])
                    const markerObject = {
                        id: pointName[i] + pointAndPolygonInfo.point[index].coordinates[i].join('-'),
                        coordinate: pointAndPolygonInfo.point[index].coordinates[i],
                        text: pointName[i]
                    }
                    if (turf.booleanPointInPolygon(pt, poly)) {
                        // __g.marker.add({ ...pointAndPolygonInfo.point[index].pointInfo, ...markerObject })
                        this.markerArr.push({ ...pointAndPolygonInfo.point[index].pointInfo, ...markerObject })
                    }
                })

            })
            setTimeout(() => {
                this.markerNameList = this.markerArr.map(item => {
                    return { text: item.text, id: item.id }
                })
                //根据id去重
                const uniqueArr = Array.from(new Set(this.markerArr.map(item => item.id))).map(id => {
                    return this.markerArr.find(item => item.id === id);
                })
                this.markerNameList = uniqueArr
                __g.marker.add(this.markerArr)
            }, 1000)
        },
        handleClickMarker(item) {
            __g.marker.focus(item.id, 30)
        },
        async selectMarker(value) {
            this.markerNameList = this.markerArr.filter(item => {
                if (iitem.text.toLowerCase().includes(value.toLowerCase())) {
                    return true
                }
            })
            const showIds = this.markerNameList.map(item => item.id)
            const hideIds = this.markerArr.map(item => item.id)
            await __g.marker.hide(hideIds)
            await __g.marker.hide(showIds)
        },
        async clearMarker() {
        ...
        async handleRedraw() {
            ...
            this.fModel.type = "";
            this.fModel.buffer = 0;
            this.fModel.tableName = "";
            this.rList = [];
            this.$forceUpdate();
        },
        // 缓冲
        handleChangeBUF() {
            if (this.fModel.type === "circular") {
                this.handleRP("add");
            }
        },
        // 查看详情
