  ...
  <div class="taskPlan_box">
    ...
    <div class="plan_box">
      <div class="screen_box" style="position: relative;">
        ...
        <!-- <div class="task-roam" @click="handleTaskPlay1">
          <span>测试自动漫游</span>
        </div> -->
        ...
        <div class="task_box">
          <div class="item_box">
            ...
            <div class="right " style="display: flex" :class="taskInfo.taskLevel==='一级加强'?'yjjq':
                  taskInfo.taskLevel==='一级任务'?'yjrw':taskInfo.taskLevel==='二级任务'?'ejrw':
                  taskInfo.taskLevel==='三级任务'?'sjrw':'bwrw'">
              <div class="point"></div>
          ...
          <!-- <div class="item_box" v-if="screenInfo.fileName && screenInfo.fileUrl"
              @click="pdfCheck(screenInfo.fileUrl, screenInfo.fileName,screenInfo.id)">
              <div class="left" style="color: #00CEFF;cursor: pointer;width: 100%;">{{ screenInfo.fileName }}</div>
            </div>
            <div class="item_box" v-else>
              <PdfUpload :id="screenInfo.id" @success="refreshData" />
            </div> -->
            <div class="item_box" @click="showDA=true">
...
import { searchNodePlanToScreen, saveScreenPlan,updateScreen,getTourDataForScreen } from "@/api/task/task";
...
// 刷新数据
const refreshData = () => {
  getTourDataForScreen({id:screenInfo.value.id}).then(res=>{
...
const handleTaskPlay1 = async() => {
 let keyFrames =  await setAutoCameraTour()
 window._cameraTour.curOa = keyFrames
  window._cameraTour.play_biz();
  playtimer.value = setTimeout(() => {
    playtimer.value = null;
  }, Number(keyFrames.time) * 1000);
}
// 播放当前任务下场景漫游
