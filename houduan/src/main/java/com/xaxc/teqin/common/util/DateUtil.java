...
public class DateUtil {
    ...
    public static boolean isADateFormat(ExcelNumberFormat numFmt) {
        return numFmt == null ? false : isADateFormat(numFmt.getIdx(), numFmt.getFormat());
    }
    ...
    private static double convertTimeInternal(String timeStr) throws Exception {
        ...
        if (len >= 4 && len <= 8) {
            ...
            switch (parts.length) {
                ...
                case 3:
                    ...
                    break;
                default:
                    throw new Exception("Expected 2 or 3 fields but got (" + parts.length + ")");
            }
    ...
    private static Date parseYYYYMMDDDateInternal(String timeStr) throws Exception {
    ...
    private static int parseInt(String strVal, String fieldName, int rangeMax) throws Exception {
        return parseInt(strVal, fieldName, 0, rangeMax - 1);
    }

    private static int parseInt(String strVal, String fieldName, int lowerLimit, int upperLimit) throws Exception {
        int result;
        try {
            result = Integer.parseInt(strVal);
        } catch (NumberFormatException var6) {
            throw new Exception("Bad int format '" + strVal + "' for " + fieldName + " field");
        }

        if (result >= lowerLimit && result <= upperLimit) {
            return result;
        } else {
            throw new Exception(fieldName + " value (" + result + ") is outside the allowable range(0.." + upperLimit + ")");
        }
    }
